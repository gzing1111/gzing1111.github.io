/* ============================================================
   ★★★ 기자재 데이터 (EQUIPMENT 배열) ★★★
   ------------------------------------------------------------
   기자재를 추가/수정/삭제하려면 이 배열만 편집하면 됩니다.
   다른 코드(검색, 필터, 정렬, 화면 렌더링)는 전부 이 배열을 자동으로 읽어서
   동작하므로 건드릴 필요가 없습니다.

   각 항목(기자재 1개)의 필드 설명:
     subject : 과목 분류 → "물리" | "화학" | "지구" | "생명" 중 하나
     room    : 위치한 실험실 → "물지실" | "화학실" (다른 실험실이면 값만 바꾸면
               필터 목록에 자동으로 추가됨)
     name    : 기자재 이름 (카탈로그에 굵게 표시됨)
     desc    : 한 줄 설명 (이름 위에 회색 글씨로 표시됨)
     tags    : 관련 실험/키워드 배열 (상세 모달에 태그로 표시됨)
     usage   : 사용법/주의사항 (상세 모달에만 표시됨)
     stock   : 보유 수량, "현재보유/전체수량" 형식 문자열 (현재는 사용 안 함,
               남겨두면 나중에 다시 활용 가능)
     location: 세부 보관 위치 (상세 모달에 표시됨)
     photo   : (선택사항) 사진 경로 또는 base64 데이터.
               넣지 않으면 자동으로 "사진 준비중" placeholder가 표시됨
   ============================================================ */
const EQUIPMENT = [
